...
export class RelatedArticlesFactory {
  ...
  getArticles () {
    ...
    if (!!tags === false || tags.length === 0) {
      console.error('RelatedArticlesFactory: Tags not provided, use setTags().')
      return [];
    }
    ...
    // (7.) For category matches, we add 3 points
    ...
    return relatedArticles.splice(0, maxArticles);
  }
}